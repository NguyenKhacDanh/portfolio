        ...
        <div style={{
          ...
          <motion.div initial={{ opacity:0, x:-30 }} whileInView={{ opacity:1, x:0 }} viewport={{ once:true }} transition={{ duration:.7 }}>
            ...
            <div style={{ marginTop:28, display:'flex', gap:12, flexWrap:'wrap' }}>
              ...
              <a href={`mailto:${profile.email}`} className="btn-ghost" style={{ fontSize:10 }}>Email</a>
            </div>
          </motion.div>

          {/* Right — stats + reflection */}
          <div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:16 }}>
              {cards(lang).map((c, i) => (
                <motion.div key={i}
                  ...
                  whileHover={{ y:-4 }}
                  className="glow-card"
                  style={{ padding:20, borderTop:`2px solid ${c.color}` }}>
                  <div style={{ position:'absolute', top:0, left:0, right:0, height:2, background:c.color, boxShadow:`0 0 12px ${c.color}` }} />
                  <div style={{ fontSize:24, marginBottom:8 }}>{c.icon}</div>
                  <div style={{ fontSize:10, letterSpacing:1.5, color:c.color, textTransform:'uppercase' }}>{c.label}</div>
                </motion.div>
              ))}
            </div>
            ...
            <motion.div initial={{ opacity:0, y:16 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ delay:.45 }}
              className="glow-card" style={{ padding:24, display:'flex', justifyContent:'space-around', marginBottom:16 }}>
              {[
                { n:'5+',  label: lang==='en'?'Years':   'Năm kinh nghiệm', color:'var(--neon)'   },
                { n:'10+', label: lang==='en'?'Systems': 'Hệ thống',         color:'var(--neon2)'  },
                { n:'20+', label: lang==='en'?'Tech':    'Công nghệ',         color:'var(--purple)' },
              ].map(s => (
                <div key={s.n} style={{ textAlign:'center' }}>
                  <div style={{ fontFamily:'Outfit,sans-serif', fontSize:34, fontWeight:900, color:s.color, lineHeight:1, textShadow:`0 0 20px ${s.color}` }}>{s.n}</div>
            ...
            <motion.div initial={{ opacity:0, y:16 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ delay:.6 }}
              className="glow-card" style={{ padding:22, borderLeft:'2px solid var(--neon2)' }}>
              ...
              <p style={{ fontSize:12, color:'var(--muted)', lineHeight:1.85, fontStyle:'italic' }}>
                {lang==='en'
                  ? '"Each year we grow a little more, and our parents walk a little further into life. Happiness isn\'t something grand — it\'s still having someone to meet, a home to return to."'
                  : '"Mỗi năm mình lớn thêm một chút, ba mẹ cũng đi qua thêm một đoạn của cuộc đời. Hạnh phúc thật ra không phải là những điều quá lớn lao — mà là vẫn còn người để gặp, còn nhà để về."'}
              </p>
            </motion.div>
          </div>
        </div>
