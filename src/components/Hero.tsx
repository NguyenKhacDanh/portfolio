...
const ticker_items = [
  'Microsoft Dynamics AX', 'D365 F&O', 'ERP Developer', 'X++ / C#',
  'Business Automation', '.NET 8', 'ASP.NET Core', 'Clean Architecture',
  'SQL Server', 'Power Automate', 'ReactJS', 'Full-Stack',
]
...
export default function Hero({ lang }: { lang: Lang }) {
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 130) {
            ...
            ctx.strokeStyle = `rgba(0,255,163,${(1 - dist / 130) * 0.13})`
            ctx.lineWidth = 0.6
            ctx.stroke()
          }
        }
  ...
  return (
    <section id="hero" style={{
      ...
      justifyContent: 'center', position: 'relative', overflow: 'hidden', paddingTop: 90,
    }}>
      {/* Particle canvas */}
      ...
      <div style={{
        ...
        backgroundImage: 'linear-gradient(rgba(0,255,163,.025) 1px,transparent 1px),linear-gradient(90deg,rgba(0,255,163,.025) 1px,transparent 1px)',
      ...
      {/* Aurora orbs */}
      <div style={{ position:'absolute', top:'8%',  right:'5%',  width:500, height:500, borderRadius:'50%', background:'radial-gradient(circle,rgba(0,255,163,.08) 0%,transparent 65%)',  animation:'float-orb 9s ease-in-out infinite',         pointerEvents:'none', zIndex:1 }} />
      <div style={{ position:'absolute', bottom:'5%', left:'2%',  width:380, height:380, borderRadius:'50%', background:'radial-gradient(circle,rgba(56,189,248,.07) 0%,transparent 65%)', animation:'float-orb 13s ease-in-out infinite reverse',  pointerEvents:'none', zIndex:1 }} />
      <div style={{ position:'absolute', top:'40%',  left:'40%', width:280, height:280, borderRadius:'50%', background:'radial-gradient(circle,rgba(192,132,252,.05) 0%,transparent 65%)', animation:'float-orb 7s ease-in-out infinite 2s',        pointerEvents:'none', zIndex:1 }} />
      ...
      <div className="container" style={{ position:'relative', zIndex:2, flex:1, display:'flex', flexDirection:'column', justifyContent:'center' }}>
        ...
        <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:.2, duration:.7 }} style={{ marginBottom:28 }}>
          <span style={{
            ...
            fontSize:11, letterSpacing:2, color:'var(--neon)', textTransform:'uppercase',
        ...
        <motion.div initial={{ opacity:0, y:40 }} animate={{ opacity:1, y:0 }} transition={{ delay:.35, duration:.8 }}>
          <h1 style={{ fontFamily:'Outfit,sans-serif', fontSize:'clamp(52px,10vw,130px)', fontWeight:900, lineHeight:.86, letterSpacing:'-5px', marginBottom:24, position:'relative', userSelect:'none' }}>
        ...
        <motion.p initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} transition={{ delay:.5, duration:.7 }}
          style={{ fontSize:12, letterSpacing:3, textTransform:'uppercase', marginBottom:20 }}
        >
        ...
        <motion.p initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:.6, duration:.7 }}
          style={{ maxWidth:520, color:'var(--muted)', lineHeight:1.9, fontSize:14, marginBottom:44 }}>
          {tr(t.hero.desc, lang)}
        </motion.p>
        ...
        <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:.65, duration:.7 }}
          ...
          {[
            { n:'4+',  label: lang === 'en' ? 'Years ERP' : 'Năm ERP', color:'var(--neon)' },
            { n:'60%+', label: lang === 'en' ? 'Automation Gain' : 'Tự động hóa', color:'var(--neon2)' },
            { n:'20+',  label: lang === 'en' ? 'Tech Stack' : 'Công nghệ', color:'var(--purple)' },
          ].map(s => (
            <div key={s.n} style={{ textAlign:'center' }}>
              <div style={{ fontFamily:'Outfit,sans-serif', fontSize:28, fontWeight:900, color:s.color, lineHeight:1, textShadow:`0 0 20px ${s.color}` }}>{s.n}</div>
        ...
        <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:1.1, duration:1 }}
          style={{ marginTop:52, padding:'18px 24px', borderLeft:'2px solid var(--neon)', background:'rgba(0,255,163,.04)', maxWidth:580, backdropFilter:'blur(6px)' }}>
          <p style={{ fontSize:12, color:'var(--muted)', lineHeight:1.75, fontStyle:'italic' }}>{tr(t.hero.quote, lang)}</p>
        </motion.div>
      </div>

      {/* Ticker */}
      <div style={{ overflow:'hidden', borderTop:'1px solid var(--border)', borderBottom:'1px solid var(--border)', padding:'14px 0', marginTop:40, background:'rgba(0,255,163,.02)', position:'relative', zIndex:2 }}>
        <div style={{ display:'flex', gap:48, animation:'ticker 32s linear infinite', width:'max-content' }}>
          {doubled.map((item, i) => (
